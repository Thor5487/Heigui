...
fun setClipboardContent(string: String) {
    try {
        ...
        Heigui.logger.error("Failed to set Clipboard Content", e)
    }
}
...
fun Module.createSoundSettings(name: String, default: String, dependencies: () -> Boolean): () -> Triple<String, Float, Float> {
    ...
    val pitch = +NumberSetting("$name Pitch", 1f, 0.1 .. 2.0, 0.01, desc = "Pitch of the sound to play.").withDependency { dependencies() }
    val volume = +NumberSetting("$name Volume", 1f, 0.1 .. 1.0, 0.01, desc = "Volume of the sound to play.").withDependency { dependencies() }
