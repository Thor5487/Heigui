...
fun modMessage(message: Any?, prefix: String = "§3Heigui §8»§r ", chatStyle: Style? = null) {
    val text = Component.literal("$prefix$message")
    chatStyle?.let { text.setStyle(chatStyle) }
    mc.execute { mc.gui.chat.addMessage(text) }
}

fun modMessage(message: Component, prefix: String = "§3Heigui §8»§r ", chatStyle: Style? = null) {
...
fun devMessage(message: Any?) {
    ...
    modMessage(message, "§3Heigui§bDev §8»§r ")
}
